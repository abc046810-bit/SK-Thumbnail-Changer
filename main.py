...
def create_stylish_thumbnail(frame_path: str, username: str, output_path: str) -> str:
    """Create a full black thumbnail with Telegram logo + @username centered.
    This makes the change 100% visible.
    """
    im = Image.open(frame_path).convert("RGB")
    width, height = im.size

    # Standard good size for Telegram video thumbnail
    max_w = 640
    if width > max_w:
        ratio = max_w / width
    ...
    logo_size = min(int(height * 0.28), 110)
    logo = logo.resize((logo_size, logo_size), Image.LANCZOS)

    # Font
    font_size = max(int(height * 0.11), 28)
    try:
    ...
    # Center everything
    total_w = logo_size + 18 + text_w
    start_x = max((width - total_w) // 2, 10)
    center_y = height // 2

    y_logo = center_y - logo_size // 2
    im.paste(logo, (start_x, y_logo), logo)

    text_x = start_x + logo_size + 18
    text_y = center_y - text_h // 2 - 4

    # Thick black outline first (for style)
    outline = 5
    for dx in range(-outline, outline + 1):
        for dy in range(-outline, outline + 1):
            if dx * dx + dy * dy <= outline * outline + 8:
                draw.text((text_x + dx, text_y + dy), text, font=font, fill=(0, 0, 0))

    # White text
    draw.text((text_x, text_y), text, font=font, fill=(255, 255, 255))

    im.save(output_path, "JPEG", quality=92, optimize=True)
    return output_path
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Force new thumbnail with file_id video
            with open(thumb_path, "rb") as thumb_f:
                await context.bot.send_video(
                    chat_id=message.chat_id,
                    video=video.file_id,
                    thumbnail=InputFile(thumb_f, filename="thumb.jpg"),
