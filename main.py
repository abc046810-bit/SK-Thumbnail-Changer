...
def create_stylish_thumbnail(frame_path: str, username: str, output_path: str) -> str:
    ...
    cover_h = max(int(height * 0.20), 56)
    draw.rectangle([0, 0, width, cover_h], fill=(6, 6, 6))

    logo = Image.open(LOGO_PATH).convert("RGBA")
    logo_size = int(cover_h * 0.60)
    logo = logo.resize((logo_size, logo_size), Image.LANCZOS)

    font_size = max(int(cover_h * 0.40), 20)
    try:
    ...
    draw.line([(0, cover_h - 1), (width, cover_h - 1)], fill=(35, 35, 35), width=2)
