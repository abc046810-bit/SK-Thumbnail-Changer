...
def create_stylish_thumbnail(frame_path: str, username: str, output_path: str) -> str:
    ...
    # Taller cover so @THEKMX type watermark fully hides
    cover_h = max(int(height * 0.28), 70)
    draw.rectangle([0, 0, width, cover_h], fill=(0, 0, 0))

    logo = Image.open(LOGO_PATH).convert("RGBA")
    logo_size = int(cover_h * 0.55)
    logo = logo.resize((logo_size, logo_size), Image.LANCZOS)

    font_size = max(int(cover_h * 0.38), 22)
    try:
    ...
    for dx in range(-outline, outline + 1):
        for dy in range(-outline, outline + 1):
            if dx * dx + dy * dy <= outline * outline + 4:
                draw.text((text_x + dx, text_y + dy), text, font=font, fill=(0, 0, 0))

    draw.text((text_x, text_y), text, font=font, fill=(255, 255, 255))
    # Thin accent line under the bar
    draw.line([(0, cover_h - 1), (width, cover_h - 1)], fill=(40, 40, 40), width=2)

    im.save(output_path, "JPEG", quality=88, optimize=True)
    return output_path
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Re-send using original file_id + new thumbnail (no big upload)
            with open(thumb_path, "rb") as thumb_f:
                await message.reply_video(
                    video=video.file_id,
                    thumbnail=InputFile(thumb_f, filename="sk_thumb.jpg"),
