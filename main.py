...
def create_stylish_thumbnail(frame_path: str, username: str, output_path: str) -> str:
    ...
    # Semi-transparent black top bar (~30% opacity).
    # Original thumbnail remains clearly visible underneath the bar.
    cover_h = max(int(height * 0.32), 80)
    overlay = Image.new("RGBA", (width, cover_h), (0, 0, 0, 77))  # ~30% opacity
    im = Image.alpha_composite(im.convert("RGBA"), overlay).convert("RGB")
    draw = ImageDraw.Draw(im)

    logo = Image.open(LOGO_PATH).convert("RGBA")
    logo_size = int(cover_h * 0.52)
    logo = logo.resize((logo_size, logo_size), Image.LANCZOS)

    font_size = max(int(cover_h * 0.36), 24)
    try:
    ...
    total_w = logo_size + 16 + text_w
    start_x = max((width - total_w) // 2, 8)
    y_logo = (cover_h - logo_size) // 2

    im.paste(logo, (start_x, y_logo), logo)

    text_x = start_x + logo_size + 16
    text_y = (cover_h - text_h) // 2 - 3

    outline = 5
    for dx in range(-outline, outline + 1):
        for dy in range(-outline, outline + 1):
            if dx * dx + dy * dy <= outline * outline + 6:
